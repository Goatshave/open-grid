...
const vueServerTreeUrl = getE2eUrl(4181);

test("vue server tree example loads async children outside core and preserves expansion", async ({ page }) => {
  await page.goto(vueServerTreeUrl);

  await expect(page.getByRole("heading", { name: "Vue server lazy tree" })).toBeVisible();
  await expect(page.getByText("18 server portfolios · page 1 of 3")).toBeVisible();
  await expect(page.getByTestId("vue-tree-expanded")).toHaveText("Expanded: none");
  await expect(page.getByTestId("vue-tree-loaded")).toHaveText("Loaded: none");
  await expect(page.locator('[role="row"][data-row-id="PFL-001"]')).toBeVisible();

  await page.getByRole("button", { name: "Toggle PFL-001" }).click();
  await expect(page.getByTestId("vue-tree-expanded")).toHaveText("Expanded: PFL-001");
  await expect(page.getByTestId("vue-tree-loading")).toHaveText("Loading: PFL-001");
  await expect(page.locator('[role="row"][data-row-id="PFL-001-loading"]')).toContainText("Loading child rows");

  await expect(page.getByTestId("vue-tree-loaded")).toHaveText("Loaded: PFL-001");
  await expect(page.getByTestId("vue-tree-loading")).toHaveText("Loading: none");
  ...
  await expect(page.getByTestId("vue-tree-sorting")).toHaveText("Sorting: budget:asc");
  await expect(page.getByTestId("vue-tree-expanded")).toHaveText("Expanded: PFL-001");
  await expect(page.locator('[role="row"][data-row-id="PFL-001-WRK-1"]')).toBeVisible();

  await page.getByRole("button", { name: /Budget/ }).click();
  await expect(page.getByTestId("vue-tree-sorting")).toHaveText("Sorting: budget:desc");
...
test("vue server tree example keeps loaded branches through pagination", async ({ page }) => {
  await page.goto(vueServerTreeUrl);
...
test("vue server tree example keeps loaded selection through collapse and re-expand", async ({ page }) => {
  await page.goto(vueServerTreeUrl);
  ...
  await expect(page.getByTestId("vue-tree-selected")).toContainText("PFL-001-WRK-1");
...
test("vue server tree example refreshes loaded lazy children in product state", async ({ page }) => {
  await page.goto(vueServerTreeUrl);
  ...
  await expect(page.locator('[role="row"][data-row-id="PFL-001-WRK-1"]')).toContainText("Portfolio 001 / work 1");
  await expect(page.getByTestId("vue-tree-loaded")).toHaveText("Loaded: PFL-001");
  await expect(page.getByTestId("vue-tree-refreshes")).toHaveText("Refreshes: none");
  ...
  await expect(page.getByTestId("vue-tree-refreshes")).toHaveText("Refreshes: PFL-001: 1");
  await expect(page.getByTestId("vue-tree-loaded")).toHaveText("Loaded: PFL-001");
  await expect(page.getByTestId("vue-tree-loading")).toHaveText("Loading: none");
  await expect(page.locator('[role="row"][data-row-id="PFL-001-WRK-1"]')).toContainText("Portfolio 001 / work 1 (refresh 1)");
});

test("vue server tree example bulk refreshes loaded lazy branches and preserves selection", async ({ page }) => {
  await page.goto(vueServerTreeUrl);
  ...
  await expect(page.getByTestId("vue-tree-loaded")).toContainText("PFL-001");
  await expect(page.getByTestId("vue-tree-loaded")).toContainText("PFL-003");
  ...
  await expect(page.getByTestId("vue-tree-selected")).toContainText("PFL-001-WRK-1");
  await expect(page.getByTestId("vue-tree-selected")).toContainText("PFL-003-WRK-1");

  await page.getByRole("button", { name: "Refresh loaded branches" }).click();
  await expect(page.getByTestId("vue-tree-expanded")).toContainText("PFL-001");
  await expect(page.getByTestId("vue-tree-expanded")).toContainText("PFL-003");
  await expect(page.getByTestId("vue-tree-loaded")).toHaveText("Loaded: none");
  await expect(page.getByTestId("vue-tree-loading")).toContainText("PFL-001");
  await expect(page.getByTestId("vue-tree-loading")).toContainText("PFL-003");
  await expect(page.getByTestId("vue-tree-selected")).toContainText("PFL-001-WRK-1");
  await expect(page.getByTestId("vue-tree-selected")).toContainText("PFL-003-WRK-1");

  await expect(page.getByTestId("vue-tree-refreshes")).toContainText("PFL-001: 1");
  await expect(page.getByTestId("vue-tree-refreshes")).toContainText("PFL-003: 1");
  await expect(page.getByTestId("vue-tree-loaded")).toContainText("PFL-001");
  await expect(page.getByTestId("vue-tree-loaded")).toContainText("PFL-003");
  await expect(page.getByTestId("vue-tree-loading")).toHaveText("Loading: none");
  await expect(page.locator('[role="row"][data-row-id="PFL-001-WRK-1"]')).toContainText("Portfolio 001 / work 1 (refresh 1)");
...
test("vue server tree example commits server-owned child mutations", async ({ page }) => {
  await page.goto(vueServerTreeUrl);

  await expect(page.getByRole("button", { name: "Mutate PFL-001 work 1" })).toBeDisabled();
  await expect(page.getByTestId("vue-tree-mutations")).toHaveText("Mutations: none");
  await expect(page.getByTestId("vue-tree-mutating")).toHaveText("Mutating: none");
  ...
  await expect(page.getByTestId("vue-tree-mutating")).toHaveText("Mutating: PFL-001-WRK-1");
  await expect(page.getByRole("button", { name: "Mutate PFL-001 work 1" })).toBeDisabled();
  await expect(page.getByTestId("vue-tree-mutations")).toHaveText("Mutations: PFL-001-WRK-1: 1");
  await expect(page.getByTestId("vue-tree-mutating")).toHaveText("Mutating: none");
...
test("vue server tree example merges server patches with local mutation policy", async ({ page }) => {
  await page.goto(vueServerTreeUrl);

  await expect(page.getByRole("button", { name: "Merge PFL-001 server patch" })).toBeDisabled();
  await expect(page.getByTestId("vue-tree-merges")).toHaveText("Merges: none");
  await expect(page.getByTestId("vue-tree-merge-conflicts")).toHaveText("Merge conflicts: none");
  ...
  await expect(page.getByTestId("vue-tree-merges")).toHaveText("Merges: PFL-001: 1");
  await expect(page.getByTestId("vue-tree-merge-conflicts")).toHaveText("Merge conflicts: PFL-001-WRK-1: kept local mutation over remote merge 1");
  await expect(page.locator('[role="row"][data-row-id="PFL-001-WRK-1"]')).toContainText("Portfolio 001 / work 1 (mutated 1)");
...
test("vue server tree example rolls back rejected optimistic child mutations", async ({ page }) => {
  await page.goto(vueServerTreeUrl);

  await expect(page.getByRole("button", { name: "Optimistic fail PFL-001 work 2" })).toBeDisabled();
  await expect(page.getByTestId("vue-tree-mutation-errors")).toHaveText("Mutation errors: none");
  ...
  await expect(page.getByTestId("vue-tree-mutating")).toHaveText("Mutating: PFL-001-WRK-2");
  await expect(workRow).toContainText("Portfolio 001 / work 2 (optimistic)");
  await expect(workRow).toContainText("Blocked");

  await expect(page.getByTestId("vue-tree-mutation-errors")).toContainText("PFL-001-WRK-2: Server rejected optimistic mutation");
  await expect(page.getByTestId("vue-tree-mutating")).toHaveText("Mutating: none");
  await expect(workRow).toContainText("Portfolio 001 / work 2");
...
test("vue server tree example keeps paginated branch mutation conflicts in product state", async ({ page }) => {
  await page.goto(vueServerTreeUrl);
  ...
  await expect(page.getByTestId("vue-tree-mutating")).toHaveText("Mutating: PFL-001-WRK-3");

  await page.getByRole("button", { name: "Next" }).click();
  await expect(page.locator('[role="row"][data-row-id="PFL-001"]')).toHaveCount(0);
  await expect(page.getByTestId("vue-tree-loaded")).toContainText("PFL-001");
  await expect(page.getByTestId("vue-tree-mutation-errors")).toContainText(
...
test("vue server tree example selects loaded descendants from expandable parents", async ({ page }) => {
  await page.goto(vueServerTreeUrl);
  ...
  await expect(page.locator('[role="row"][data-row-id="PFL-001-WRK-4"]')).toHaveAttribute("data-selected", "true");
  await expect(page.getByTestId("vue-tree-selected")).toContainText("PFL-001-WRK-1");
  await expect(page.getByTestId("vue-tree-selected")).toContainText("PFL-001-WRK-4");
...
test("vue server tree example keeps async load errors and retries in product state", async ({ page }) => {
  await page.goto(vueServerTreeUrl);

  await expect(page.getByTestId("vue-tree-errors")).toHaveText("Errors: none");
  await expect(page.getByRole("button", { name: "Retry PFL-002" })).toBeDisabled();

  await page.getByRole("button", { name: "Expand PFL-002" }).click();
  await expect(page.getByTestId("vue-tree-loading")).toHaveText("Loading: PFL-002");
  await expect(page.locator('[role="row"][data-row-id="PFL-002-loading"]')).toContainText("Loading child rows");

  await expect(page.getByTestId("vue-tree-errors")).toContainText("PFL-002: Temporary server error");
  ...
  await expect(page.getByTestId("vue-tree-loaded")).toContainText("PFL-002");
  await expect(page.getByRole("button", { name: "Retry PFL-002" })).toBeDisabled();
});

test("vue server tree example cancels collapsed loads and ignores stale responses", async ({ page }) => {
  await page.goto(vueServerTreeUrl);

  await expect(page.getByTestId("vue-tree-cancelled")).toHaveText("Cancelled: none");

  await page.getByRole("button", { name: "Expand PFL-003" }).click();
  await expect(page.getByTestId("vue-tree-expanded")).toHaveText("Expanded: PFL-003");
  await expect(page.getByTestId("vue-tree-loading")).toHaveText("Loading: PFL-003");
  ...
  await expect(page.getByTestId("vue-tree-expanded")).toHaveText("Expanded: none");
  await expect(page.getByTestId("vue-tree-loading")).toHaveText("Loading: none");
  await expect(page.getByTestId("vue-tree-cancelled")).toContainText("PFL-003: collapsed before response");
  await expect(page.locator('[role="row"][data-row-id="PFL-003-loading"]')).toHaveCount(0);

  await page.waitForTimeout(250);
  await expect(page.getByTestId("vue-tree-loaded")).not.toContainText("PFL-003");
