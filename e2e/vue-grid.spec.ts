...
const vueGridUrl = getE2eUrl(4174);
const vueServerGridUrl = getE2eUrl(4179);

test("vue data grid downloads CSV exports", async ({ page }) => {
  await page.goto(vueGridUrl);
...
test("vue data grid applies product-owned root, row, header, and cell classes", async ({ page }) => {
  await page.goto(vueGridUrl);
...
test("vue data grid exposes product-owned column management controls", async ({ page }) => {
  await page.goto(vueGridUrl);
...
test("vue data grid manages column visibility from the shared controls", async ({ page }) => {
  await page.goto(vueGridUrl);
...
test("vue data grid changes row density from the shared controls", async ({ page }) => {
  await page.goto(vueGridUrl);
...
test("vue data grid restores and resets persisted product preferences", async ({ page }) => {
  await page.goto(vueGridUrl);
...
test("vue data grid exposes richer header action menus", async ({ page }) => {
  await page.goto(vueGridUrl);
  ...
  await expect(menu.getByTestId("vue-menu-custom-slot")).toHaveText("Width token: owner");
  ...
  await expect(page.getByTestId("vue-design-menu-action")).toHaveText("Design menu: none");
  ...
  await expect(page.getByTestId("vue-design-menu-action")).toHaveText("Design menu: owner width token 180");
...
test("vue data grid disables direct header pinning controls for the active pin state", async ({ page }) => {
  await page.goto(vueServerGridUrl);
...
test("vue data grid resizes a column with keyboard arrows and Home/End", async ({ page }) => {
  await page.goto(vueGridUrl);
...
test("vue data grid selects leaf rows from grouping-panel grouped rows", async ({ page }) => {
  await page.goto(vueGridUrl);
...
test("vue data grid keeps grouped virtualization bounded at aggregate footers", async ({ page }) => {
  await page.goto(vueGridUrl);
  ...
  await expect.poll(async () => scroller.locator(".og-grid__body").evaluate((element) => element.getBoundingClientRect().height)).toBeGreaterThan(5_000);
...
test("vue data grid keeps grouping panel reorder virtualization bounded at aggregate footers", async ({ page }) => {
  await page.goto(vueGridUrl);
...
test("vue data grid supports sorting, keyboard navigation, selection, and resizing", async ({ page, context }) => {
  ...
  await expect(page.getByTestId("vue-server-edit-status")).toHaveText("Server edit: idle");
  ...
  await cityEditor.press("Enter");
  await expect(page.getByTestId("vue-server-edit-status")).toHaveText("Server edit: saving REG-001 city");
  await expect(cityEditor).toBeVisible();
  await expect(page.getByTestId("vue-server-edit-status")).toHaveText("Server edit: saved REG-001 city = Seoul Edited");
  ...
  await expect(page.getByTestId("vue-server-edit-status")).toHaveText("Server paste: saving REG-001 city");
  await expect(page.getByTestId("vue-server-edit-status")).toHaveText("Server paste: saved REG-001 city = Busan Server");
